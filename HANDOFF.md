...
- T043（Class M、統合済み）では、private-marker scanner がpathだけをindexから列挙してworking treeだけを読むため、stage済みmarkerを見落とし得た経路を修正した。変更前はstaged marker＋clean working treeの2 assertionが期待どおりRED。raw NUL-delimited `ls-files --stage` / porcelain v2を両PowerShell hostで解析し、index blobとtracked working-tree bytesを検査する。missing / unsafe path、intent-to-add、unmerged / non-regular mode、invalid UTF-8、Git failureをredacted messageでfail closedにし、exact worktree bytes再読を2回のGit snapshot照合で挟む。独立review 2系統で見つかったworktree/index両方向のTOCTOUはinstrumented race fixtureで修正し、最終P0–P3 CLEAR。PowerShell 7 / Windows PowerShell 5.1のfinal check:all 6本、Gitleaks staged 34.22 KB、Semgrep 82 rules / 29 filesは全pass。feature commit `8e58b33`はPR #59、merge `e4106f8`で統合し、PR run `30809598740`とmain run `30809711691`はsuccess、各annotation 0。feature branchはlocal / remoteともに削除済み。release / tag / workflow / permissionは変更していない。
- T042（Class S）では、T041で追加した`SKILL.md` frontmatter canonical contractと番号付き7軸の順序照合を、v0.2.0のGitHub Release本文案へ同期した。`CHANGELOG.md`の実装済み記述を根拠にし、リリース最終化手順・ゲート①承認依頼・tag・Release・workflow・permissionは変更していない。変更前baselineと変更後のPowerShell 7 / Windows PowerShell 5.1 check:all 6コマンドはpassし、global pre-commit Gitleaksもpassした。Semgrepは対象ファイルなしでskip。PR #58。
- T040（Class M）では、`AGENTS.md` §3 / §5 に残っていた branch・release・issue / PR・doing・CI の時点依存snapshotを、remote配線、worktree / stash、local / remote branch・tag、GitHub状態を着手時に再実測する契約へ置き換えた。初期4 assertion時点の旧文書は期待どおり4件RED。最終public-readinessはvisible §3の契約行をexactに固定し、時点依存断定を拒否してfence / comment decoyを識別するtable-driven 17件と、§5のH2境界・evergreen冒頭を固定する5件をfail closedに検査する。PowerShell 7 / Windows PowerShell 5.1 の check:all 6コマンドはpass。`CHANGELOG.md`とv0.2.0本文案も同期したが、release / tag / workflow / permission は変更していない。
- T041（Class M、統合済み）では、中核成果物 `SKILL.md` のfrontmatterと7軸が存在確認だけでdriftし得る経路をpublic-readinessへ追加した。初期stubに対し、frontmatter、description、axis同期の負例15件が期待どおりRED。実装直後の24件、trigger・必須field・両文書同時改名を加えた28件は両PowerShell hostでpass。独立reviewで除外文の極性反転と不正backtick info stringによる追加軸隠蔽を検出し、追加3負例のRED後にcanonical description完全一致とCommonMark fence判定へ修正した。続くreviewでは同一行終端raw HTMLとType 7 paragraph中断誤判定による軸隠蔽、block comment終端suffixの偽axis算入を検出。SKILL/checklist向け計8負例のRED後、block commentを専用stateで除外し、それ以外のraw HTMLは部分解釈せず固定sentinelでfail closedにした。さらに複数行inline commentの終端suffixでcanonical H2を合成できる経路を2負例のRED後に固定sentinelへ倒した。最終41件とproduction文書、両hostのfull check:all 6コマンドはpass。Gitleaks worktree / 報告上66 commitsは0件、Semgrep 82 rules / 29 filesも0 findings。feature commit `6159992`はPR #56、merge `6545419`で統合し、PR run `30685669592`とmain run `30685691011`はsuccess、各check annotationは0件。feature branchとworktreeの不存在も確認済み。release / tag / workflow / permission は変更していない。
...
- T043のunmerged stage 1/2/3を人工生成する追加fixtureは、Git index setupが3回成立しなかったため同じfailure classの再試行を停止した。production parserはstage 0以外を固定messageで拒否し独立reviewはCLEARだが、このfixtureの実行証拠は未確認。native Linux / macOSとPSScriptAnalyzerも未確認。
