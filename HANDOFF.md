...
## 確認済みスナップショット（2026-07-26 JST 実測）

- 既定ブランチ `main`。T001–T027 が完了済み。T022 では checklist 実項目数と軸数を README の数値表記と固定値なしで相互比較する public-readiness 検証を追加。
...
- T027 着手時の open issue / open PR: 0件。doing タスク: 0件。
- 最新の tag と GitHub Release は `v0.1.0`。
- `references/checklist.md` は実測80項目・7節で README の表記と一致。
- check:all 3本（§7）と Windows PowerShell 5.1 互換実行は 2026-07-26 に pass。T027 着手前の `main` CI（`validation.yml`、commit `d823201`）も緑。
