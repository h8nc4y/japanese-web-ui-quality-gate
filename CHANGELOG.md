...
- The private-marker scanner now defaults to git-tracked files and inspects both staged index blobs and current tracked working-tree bytes, closing the staged/worktree divergence gap. Raw NUL-delimited Git output preserves record boundaries across PowerShell hosts; exact working-tree bytes/path safety are revalidated between two final Git index/status checks. Missing, unsafe, or concurrently changed paths, unmerged / intent-to-add / non-regular index entries, unreadable text-like blobs, invalid UTF-8 text, and index/status drift fail closed with redacted diagnostics. This is bounded optimistic drift detection rather than a cross-process lock. The scanner skips binary-like files, reports line numbers, adds AWS/GCP/Slack/Stripe/PEM coverage, and avoids treating `task-scanner`-style slugs as OpenAI-style tokens. Its regression harness reuses the current PowerShell executable so Windows PowerShell 5.1 checks no longer delegate child scanner runs to PowerShell 7.
...
- Public-readiness validation now checks the central `SKILL.md` frontmatter contract and compares its ordered seven numbered axes with `references/checklist.md`. Missing, malformed, or duplicated metadata, any canonical trigger or exclusion drift (including polarity reversal), and missing, extra, renamed, or reordered axes fail closed. Valid fenced examples, HTML block comments, and same-line inline comments remain outside the contract; invalid backtick info strings cannot hide an added axis, while non-comment raw HTML and multiline inline comments are rejected instead of being partially interpreted.
...
- The Validation workflow contract now also fixes the exact workflow name, pull-request and `main` push events, and `contents: read` permission. Focused fixtures reject manual/scheduled triggers, additional branches or tags, missing or expanded permissions, and duplicate keys without changing the workflow, release, or tag behavior.
- The Validation workflow now pins the verified `actions/checkout` v7.0.1 commit, replacing the Node.js 20-based v4.4.0 pin that emitted deprecation annotations. The exact contract rejects mutable v7, the stale v4.4.0 pin, an incorrect SHA, and a missing version comment while retaining credential non-persistence and the existing workflow shape.
...
- `AGENTS.md` now treats current Git / GitHub state as live evidence instead of embedding a branch, release, issue, task, or CI snapshot. Public-readiness extracts the visible section 3 contract, requires the local/remote/WIP measurement set, and rejects point-in-time version, SHA, count, branch, release, or CI claims while ignoring fenced and commented examples.
