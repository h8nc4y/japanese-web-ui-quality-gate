...
- public-readiness は`SKILL.md`のfrontmatter canonical contractと、`SKILL.md` / `references/checklist.md`の番号付き7軸を順序込みで照合します。必須metadata、trigger、除外条件、軸の欠落・追加・改名・並べ替えがdriftした場合はfail closedに拒否し、fenceやHTML commentの例示は契約へ数えません。安全に解釈できないraw HTMLや複数行inline commentで軸構造を隠せる場合も、部分解釈せず失敗させます。
